  ...
  return (
    <DashboardLayout title="Job Matches">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-foreground font-serif">Smart Job <span className="warm-text">Matches</span></h2>
          <p className="text-sm text-muted-foreground mt-1">AI-matched jobs based on your resume and skills</p>
        </div>
            ...
            <div key={i} className="glass-card rounded-xl p-6 hover:border-primary/20 transition-all">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="text-right">
                  <div className={`text-lg font-bold ${job.match >= 90 ? "text-primary" : "text-foreground"}`}>{job.match}%</div>
              ...
              <div className="flex flex-wrap gap-2">
                {job.skills.map((s, j) => <span key={j} className="px-2 py-1 rounded-full bg-primary/10 text-primary text-xs">{s}</span>)}
              </div>
            </div>
          ))}
        </div>
